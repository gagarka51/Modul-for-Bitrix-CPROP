...
class CIBlockPropertyCProp
{
	...
	public function GetLength($arProperty, $arValue, $additionalValue)
	{
		$arFields = self::prepareSettings(unserialize($arProperty['USER_TYPE_SETTINGS']));
    ...
    {
        $arFields = self::prepareSettings($arProperty['USER_TYPE_SETTINGS']);
                        ...
                        <td>
                            <table class="mf-img-table">
                                <tr>
                                    <td>'.$content.'<br>
                                        <div>
                                            <label><input name="'.$strHTMLControlName['VALUE'].'['.$code.'][DEL]" value="Y" type="checkbox"> '. Loc::getMessage("SHARK_CPROP_FILE_DELETE") . '</label>
    ...
    public static function showHtmlEditor($code, $title, $arValue, $strHTMLControlName)
    {
		...
		$result .= '<input type="hidden" name="' . $strHTMLControlName["VALUE"] . '[' . $code . ']'.'"value="'.$v.'">';
	...
	{
		$hideText = Loc::getMessage('SHARK_CPROP_HIDE_TEXT');
        $clearText = Loc::getMessage('SHARK_CPROP_CLEAR_TEXT');
	...
	public static function GetSettingsHTML($arProperty, $strHTMLControlName, &$arPropertyFields)
    {
        ...
        $settingsTitle =  Loc::getMessage('SHARK_CPROP_SETTINGS_TITLE');
        ...
        $result = '<tr><td colspan="2" align="center">
            <table id="many-fields-table" class="many-fields-table internal">        
                <tr valign="top" class="heading mf-setting-title">
                   <td>XML_ID</td>
                   <td>'.Loc::getMessage('SHARK_CPROP_SETTING_FIELD_NAME').'</td>
                   <td>'.Loc::getMessage('SHARK_CPROP_SETTING_FIELD_SORT').'</td>
                   <td>'.Loc::getMessage('SHARK_CPROP_SETTING_FIELD_TYPE').'</td>
                </tr>';


        $arSetting = self::prepareSettings($arProperty['USER_TYPE_SETTINGS']);
	...
	public static function prepareSettings($arSetting)
	{
		$arResult = [];
